...
@Component({
  ...
  templateUrl: './acidez-vino.page.html',
  styleUrls: ['./acidez-vino.page.scss'],
})
export class AcidezVinoPage implements OnInit {
    ...
    $.ajax({
      ...
      success: function (datosGrafica) {
        ...
        $.each(datosGrafica, function (key, value) {
          ...
          if (value.acidez) { datosGrafica[key].acidez = parseFloat(value.acidez); }

          acidez.push([value.fecha, value.acidez]);
        }); 
   chart = Highcharts.chart('acidez',{
    chart: {
    type: 'spline',
